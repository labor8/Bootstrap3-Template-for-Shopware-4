...
(function($) {
    $(document).ready(function() {
        ...
        slider.find('.sliding_articles').css('height', {$sliderHeight});
        slider.find('.ajaxSlider').css('height', {$sElementHeight-2});
    });

})(jQuery);
</script>
<div class="slider_article_{$Data.objectId}">
    <pre>perPage: {$perPage} | pages: {$Data.pages}</pre>
    {if $Data.article_slider_type == 'selected_article'}
    {foreach $Data.values|array_chunk:$perPage as $articles}
        {include file="widgets/emotion/slide_articles.tpl" articles=$articles sElementWidth=$sElementWidth sPerPage=$perPage}
    {/foreach}
    {/if}
</div>